...
(function($) {
		...
		cancel : function(fileID, supressEvent) {
			...
			this
					.each(function() {
						...
						$('#file_upload').uploadify('settings','uploadLimit', ++uploadLimit);//重置增加上传文件数
						if (args[0]) {
							// Clear the queue
							if (args[0] == '*') {
	...
	var handlers = {
		...
		onDialogClose : function(filesSelected, filesQueued, queueLength) {
			...
			if ($.inArray('onDialogClose', settings.overrideEvents) < 0) {
				if (this.queueData.filesErrored > 0) {
					console.info(this.queueData.errorMsg);
				}
			}
		...
		onSelect : function(file) {
			...
			var queuedFile = {};
			//console.info(this.queueData.files);
			for ( var n in this.queueData.files) {
				queuedFile = this.queueData.files[n];
				//console.info(queuedFile);
				//console.info("1--");
				//console.info(file);
			...
			//console.info(settings.overrideEvents);
			// Run the default event handler
		...
		onUploadError : function(file, errorCode, errorMsg) {
			...
			case SWFUpload.UPLOAD_ERROR.UPLOAD_LIMIT_EXCEEDED:
				console.info('The upload limit has been reached (' + errorMsg + ').');
